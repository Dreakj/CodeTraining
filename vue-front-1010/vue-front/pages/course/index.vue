...
export default {
  ...
  methods: {
    ...
    searchOne(subjectParntId, index) {
      //把传递index值赋值给oneIndex，为了active样式生效
      this.oneIndex = index;
      this.twoIndex = -1;
      this.searchObj.subjectId = "";
      this.subSubjectList = [];
      //把点击的一级分类的id值赋值给searchObj
      this.searchObj.subjectParntId = subjectParntId;
      this.gotoPage(this.page);
      for (let i = 0; i < this.subjectNestedList.length; i++) {
        //获取每个一级分类和点击的类别id是否相同
        //如果相同取出其二级分类
        if (this.subjectNestedList[i].id === subjectParntId) {
